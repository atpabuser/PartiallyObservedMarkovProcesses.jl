...
@info h1("slice and profile tests")

@testset verbose=true "slice and profile" begin
    ...
    @testset "profile" begin
        ...
        pr = profile(
            P,d;
            Nmif=3,Np=50,rw_sd=(k=0.05,x0=0.05),
            transform=(k=:log,x0=:log),cooling_fraction_50=0.5,
            nreps=2,Np_eval=100,
        )
        ...
        @test pr2 == pr
        ## perturbing the profiled parameter is refused
        @test_throws r"must not be perturbed" profile(P,d;Nmif=1,Np=10,rw_sd=(a=0.1,k=0.1))
        @test_throws r"must not be perturbed" profile(
            P,d;Nmif=1,Np=10,rw_sd=(k=0.1,),rw_sd_init=(a=0.1,)
        )
        ## a plain data frame without metadata is accepted as a design
        pr3 = profile(P,DataFrame(a=[1.5],k=[7.0],x0=[5.0]);Nmif=1,Np=20,rw_sd=(k=0.05,))
        @test nrow(pr3) == 1
        @test pr3.x0 == [5.0]
        ...
        pr4 = profile(P,DataFrame(a=[1.5]);Nmif=1,Np=20,rw_sd=(k=0.05,))
