...
@testset verbose=true "Gompertz: profile against the exact profile likelihood" begin
    ...
    pr = profile(
        P,d;
        Nmif=40,Np=1000,
        perturbations=@perturbn(@lognormal(σₚ,0.1)),
...
@testset verbose=true "Gompertz: every resampling setting is unbiased" begin

    ## Triggered and power-tempered resampling change the variance of the
    ## likelihood estimate, not its target: for each setting the
    ## likelihood estimate itself (not its logarithm) must have mean
    ## equal to the exact likelihood. σₘ = 0.3 keeps the effective sample
