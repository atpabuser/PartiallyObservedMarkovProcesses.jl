...
divergence. Two further, still-live differences are documented: an unequally
weighted terminal ancestry draw, which Aaron's code initiates uniformly
rather than proportional to the terminal weights, and the presence, in
Debsurya's translation only, of a per-step resampling diagnostic. It closes
...
Normalizing constant $C$ at $0<\beta<1$ (\S\ref{sec:normconst})
  & dropped \newline\emph{(bug: biases $\hat L$ low by $\Theta(1/n)$ per resampling step)}
  & restored: \texttt{logLik[] += log C} \\[0.4em]
Terminal ancestry draw (\S\ref{sec:ancestry})
  & uniform \newline\emph{(unchanged at \texttt{1d8dbc0})}
