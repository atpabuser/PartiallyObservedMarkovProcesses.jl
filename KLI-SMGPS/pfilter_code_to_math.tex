...
constant $C$ whenever $0<\beta<1$: Section~\ref{sec:normconst} derives $C$
...
routine (Section~\ref{sec:normconst}).
...
not merely a bookkeeping convenience: Section~\ref{sec:normconst} shows that
the quantity
\[
C = \frac{m\cdot S'}{J}
\]
...
it out. Proposition~\ref{prop:normconst} below records exactly when $C$ is
forced to $1$ and when it is not.

\begin{proposition}
\label{prop:normconst}
...
\begin{corollary}
\label{cor:extremes}
$C$ of Proposition~\ref{prop:normconst} is identically $1$ whenever the filter
...
Proposition~\ref{prop:normconst}(c) shows $\E[C\mid w^{1:J}]=1$ for every
...
exactly zero at $\beta\in\{0,1\}$. Section~\ref{sec:exact-case} makes
...
Proposition~\ref{prop:normconst}(c).
...
Section~\ref{sec:normconst}. On exit, \texttt{w} is renormalized to unit mean
...
$\beta=$~\texttt{target} (line 315); as Section~\ref{sec:normconst} works out
...
resampling exactly, $C$ of Proposition~\ref{prop:normconst}(a) being
...
Proposition~\ref{prop:normconst}(a) already guarantees that the resampling
bookkeeping question examined in Section~\ref{sec:normconst} — whether the
...
the other; Section~\ref{sec:normconst} settles it instead with an exact
...
(Section~\ref{sec:exact-case}), since the present check's model —
...
This document did not arrive at Section~\ref{sec:normconst} in one pass. The
...
The third is the subject of Section~\ref{sec:normconst}: the normalizing
...
Proposition~\ref{prop:normconst}(c) gives $\E[C\mid w^{1:J}]=1$ exactly, so
...
two-particle case of Section~\ref{sec:exact-case}, $\E[C]=1$
...
Section~\ref{sec:exact-case}: at $J=2$ the systematic sweep is
